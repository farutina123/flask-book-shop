...
def not_log():
    with session_scope() as session:
        table_books = session.query(Book).first()
    if not table_books:
...
def group_of_genre_home(title_genre):
    with session_scope() as session:
        books_list = session.query(Book).filter_by(genre=title_genre).all()
        genre_list = session.query(Genre)
        if books_list:
            return render_template('genre/genre_home.html', photo=book_photo,
                                   genres=genre_list, title=title_genre, books_list=books_list)
    return Response(
        status=404,
    )
...
def group_of_genre(title_genre):
    with session_scope() as session:
        ...
        genre_list = session.query(Genre)
        if books_list:
            return render_template('genre/genre_not_log.html', photo=book_photo,
                                   genres=genre_list, title=title_genre, books_list=books_list)
    return Response(
        status=404,
    )
